...
public class NetUtils {
	...
	public static String getGetResult(Map<String, Object> params,String targetURL){
		String result = null;
		try {
			String httpParams = generateHttpParams_get(params);
			targetURL += httpParams;
			HttpGet get = new HttpGet(targetURL);
			get.setHeader(HTTPPOST_HEADER_NAME, HTTPPOST_HEADER_CONTENT);
			HttpConnectionParams.setConnectionTimeout(new BasicHttpParams(), WAIT_TIME);
			HttpResponse response = MySSLSocketFactory.getDefaultHttpClient().execute(get);
	...
	private static String generateHttpParams_get(Map<String, Object> params) {
		...
		if (params == null) {
			...
			for (String key : params.keySet()) {
				...
				try {
					value_URL = URLEncoder.encode(value.toString(), "utf-8");
				} catch (UnsupportedEncodingException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				result += (key + "=" + value_URL + "&");
			}
			return result;
		}
	}
	...
	public static String getPostResult(Map<String, Object> params,String targetURL){
		...
		try {
			...
			List<NameValuePair> httpParams = generateHttpParams_post(params);
	...
	private static List<NameValuePair> generateHttpParams_post(
