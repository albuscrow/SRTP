...
import edu.cqu.srtp.adapter.MyPagerAdapter;
import edu.cqu.srtp.adapter.UpdateListAdapter;
import edu.cqu.srtp.common.AppConfig;
...
import edu.cqu.srtp.listener.MineOnClickListener;
import edu.cqu.srtp.listener.MyOnPageChangeListener;
import edu.cqu.srtp.listener.MyOnTabChangeListener;
import edu.cqu.srtp.listener.MyTextViewOnClickListener;
import edu.cqu.srtp.util.HistoryManager;
import edu.cqu.srtp.util.TitleManeger;
import edu.cqu.strp.view.MyListview;
...
public class MainActivity extends Activity{
	...
	private void initSearchView() {
		...
		searchItemListView = (ListView)findViewById(R.id.searched_item_list);
		...
		doSearch.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent=new Intent(MainActivity.this,SearchedBooksActivity.class);
				String keyword=editText.getText().toString();
				if(null==keyword || keyword.length()<1){
					Toast.makeText(getApplicationContext(), "请输入关键字！", Toast.LENGTH_SHORT).show();
					return;
				}
				intent.putExtra(SearchedBooksActivity.INTENT_NAME, keyword);
	...
	private void initItemDisplayViewPager() {
		...
		MyListview recommendController = new MyListview(this,0);
		View recommend=recommendController.getRoot();
		
		MyListview popularController = new MyListview(this,1);
